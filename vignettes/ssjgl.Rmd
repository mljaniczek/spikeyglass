...
simdat$Y <- simdat$Y[c(1,2)]
simdat$g <- simdat$g[c(1,2)]
set.seed(1219)
G <- 2
p <- 50
penalty <- "fused"
...
# takes 1 minute to run the below
library(tictoc)
tic()
...
plot_path(lam1/v0s, fit1, thres = 0, normalize = F, xlab = expression(lambda[1]/v[0]^2), ylab = "Partial correlations",  main = mains, par = c(1, 2))


```


Now with new v0 selection
```{r}
...
# TODO make function to use CIs to 

# TODO make function to use CIs to do something with differential edges - like if CIs don't overlap between groups call them "differential"

# TODO parallelize bootstrap code to speed up?

# make same sort of functions for 
